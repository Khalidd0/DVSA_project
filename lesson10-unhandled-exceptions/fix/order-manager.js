    ...
    try {
        ...
        userData.then((userData)=>{
            ...
            if (isOk == true) {
               ...
               // });
               lambda_client.send(command).then((lambda_response) => {
                const rawPayload = Buffer.from(lambda_response.Payload).toString();
            
                if (lambda_response.FunctionError) {
                    console.log("Downstream Lambda error:", rawPayload);
                    return callback(null, resp(500, {
                        status: "err",
                ...
                try {
                    ...
                    console.log("Invalid downstream response:", rawPayload);
                    return callback(null, resp(500, {
                        status: "err",
                        msg: "internal server error"
                    }));
